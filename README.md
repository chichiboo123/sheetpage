...
- **가운데** — 선택한 시트. 상단에 `전체 시트 › 시트명` 위치 표시와 이전/다음 시트
  ...
  있습니다.
  - **표 보기** — 편집용 그리드. 셀 선택 · 수정 · 수식 입력 · 병합 셀 표시. 머리글
    행은 스크롤해도 고정되고, 열 경계를 끌어 너비를 조절할 수 있습니다(더블클릭하면
    원래 너비).
...
열어둔 Workbook은 IndexedDB에 저장되어 새로고침해도 보던 시트와 수정 내용까지 그대로
복원됩니다. localStorage가 아닌 이유는 스프레드시트가 수 MB 단위이고, localStorage는
그것을 쓰는 동안 UI를 멈추는 동기 문자열 저장소이기 때문입니다.

레코드는 **브라우저 탭 단위**로 저장됩니다. 그렇지 않으면 서로 다른 파일을 연 두 탭이
같은 자리를 덮어써서, 한쪽을 새로고침했을 때 다른 쪽 문서가 나타납니다. 저장은 항상
best-effort이며(시크릿 창 · 저장소 차단 · 용량 초과), 실패해도 앱 동작에는 영향이
없습니다. 초기화하면 저장본도 함께 지워집니다.
...
- **브라우저에 남는 사본** — 열어둔 파일은 새로고침 복원을 위해 그 브라우저의
  IndexedDB에 남습니다. 공용 PC에서 민감한 문서를 봤다면 초기화로 지워주세요.
