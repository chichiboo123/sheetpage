...
- **왼쪽 사이드바** — Workbook 안의 모든 시트를 페이지 목록처럼 표시. 검색, 현재
  시트 강조, 접기/펼치기. 모바일에서는 Drawer로 전환됩니다.
- **가운데** — 선택한 시트. 두 가지 읽기 방식을 오갈 수 있습니다.
  - **표 보기** — 편집용 그리드. 셀 선택 · 수정 · 수식 입력 · 병합 셀 표시.
  - **읽기 보기** — 읽기용 레이아웃. 줄바꿈되는 텍스트, 자동 행 높이. 열이 많은
    시트는 행마다 `라벨 · 값` 블록으로 펼쳐 보여줍니다.
- **상단 Toolbar** — 로고, Workbook 제목과 출처, 실행 취소, 공유, 다운로드.
